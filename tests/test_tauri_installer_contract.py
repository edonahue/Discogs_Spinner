...
def test_installer_workflow_runs_sidecar_and_platform_bundle_validators():
    ...
    for marker in (
        ...
        'raw_dir="dist/release/raw"',
        'publish_dir="dist/release/publish"',
        "Download all installer artifacts",
        "merge-multiple: false",
        "Stage publish assets",
        "mapfile -d '' -t gtk_debs",
        "find \"$RELEASE_RAW_DIR\"/installer-gtk4-deb -type f -name '*.deb' -print0",
        "mapfile -d '' -t linux_debs",
        "find \"$RELEASE_RAW_DIR\"/installer-linux-* -type f -name '*.deb' -print0",
        "find \"$RELEASE_RAW_DIR\"/installer-linux-* -type f -name '*.AppImage' -print0",
