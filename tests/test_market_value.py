...
def test_market_value_examples_returns_high_and_low_artist_album_rows(isolated_xdg):
    conn = get_connection()
    try:
        upsert_releases(conn, [_release(10), _release(20), _release(30), _release(40, is_active=0)])
        upsert_market_price(
            conn,
...
def test_market_value_missing_lists_active_unpriced_releases(isolated_xdg):
    conn = get_connection()
    try:
        upsert_releases(conn, [_release(61), _release(62), _release(63), _release(64, is_active=0)])
        upsert_market_price(
            conn,
...
def test_market_value_missing_csv_export_writes_reason_and_market_fields(isolated_xdg, tmp_path):
    conn = get_connection()
    try:
...
def test_refresh_market_values_updates_missing_and_stale_entries(isolated_xdg, monkeypatch):
    now = datetime.now(timezone.utc)
    stale_iso = (now - timedelta(days=40)).isoformat()
    fresh_iso = (now - timedelta(days=2)).isoformat()
    ...
    class _FakeClient:
        ...
        def fetch_market_price_suggestions(self, discogs_release_id: int):
            if discogs_release_id == 10:
                return {"lowest": 10.0, "median": 11.0, "highest": 14.0, "currency": "USD"}
            if discogs_release_id == 30:
                return {"lowest": 30.0, "median": 35.0, "highest": 40.0, "currency": "USD"}
...
def test_refresh_market_values_continues_on_per_release_api_errors(isolated_xdg, monkeypatch):
    conn = get_connection()
    try:
...
def test_refresh_market_values_release_ids_targets_specific_active_releases(
    ...
    conn = get_connection()
    try:
        upsert_releases(conn, [_release(101), _release(102), _release(103, is_active=0)])
    finally:
        conn.close()
    ...
    monkeypatch.setattr(value_refresh, "DiscogsClient", _FakeClient)
    summary = run_refresh_market_values(limit=1, stale_days=365, release_ids=[102, 999, 103, 101])
...
def test_refresh_market_values_from_missing_uses_backlog_selection(
    ...
    conn = get_connection()
    try:
        upsert_releases(conn, [_release(201), _release(202), _release(203), _release(204)])
        upsert_market_price(
            conn,
    ...
    try:
        ...
        assert get_market_price(conn, 202) is not None and get_market_price(conn, 202)["median"] == 203.0
        assert get_market_price(conn, 203) is not None and get_market_price(conn, 203)["median"] == 204.0
        assert get_market_price(conn, 204) is not None and get_market_price(conn, 204)["median"] == 5.0
    finally:
        conn.close()
...
def test_refresh_market_values_rejects_from_missing_with_release_ids():
    with pytest.raises(ValueError, match="Cannot combine from_missing"):
        run_refresh_market_values(limit=10, stale_days=30, release_ids=[1], from_missing=True)
