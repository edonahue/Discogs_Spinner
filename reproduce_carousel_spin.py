...
def test_restart_carousel():
    print("Initializing carousel...")
    ...
    print(f"Spin 1 source ID: {carousel._center_spin_source_id}")
    
    # Tick for a while (simulate spinning)
    for _ in range(5): tick_all()
    
    # Set target
    print("Spin 1: Set Target to 5")
    ...
    if carousel._center_spin_source_id is not None:
        ...
        # Force stop for next test
        carousel.stop_center_spin_animation()
    else:
        print("Spin 1 finished cleanly.")
        
    # Verify index
    if carousel._index != 5:
        print(f"FAIL: Spin 1 landed on {carousel._index} instead of 5")
    ...
    print(f"Spin 2 source ID: {carousel._center_spin_source_id}")
    
    # Tick for a while
    for i in range(5):
        tick_all()
        if carousel._center_spin_source_id is None:
            print(f"FAIL: Spin 2 stopped prematurely at tick {i}")
            return
